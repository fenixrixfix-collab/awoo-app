...
function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isVolunteer && !formData.helpDescription) { setError('Опишите проблему'); return; }
    setError('');
    setLoading(true);
  ...
  return (
    <div className="create-post-page">
      ...
      <form onSubmit={handleSubmit} className="form-container">
        ...
        {/* Вознаграждение — только для "потерял" */}
        {postType === 'lost' && (
          <div className="section">
        ...
        {/* Блок волонтёра — описание проблемы и срочность */}
        {isVolunteer && (
          <div className="section" style={{border:'2px solid #E8F5E9', borderRadius:'12px', background:'#F9FFF9'}}>
            <div className="section-title" style={{color:'#388E3C'}}>🤝 Нужна помощь</div>

            <div className="form-field">
              <label className="form-label required" style={{color:'#388E3C'}}>Описание проблемы</label>
              <textarea
                name="helpDescription"
                className="form-input"
                placeholder="Опишите ситуацию и какая именно помощь нужна..."
            ...
            <div className="form-field">
              <label className="form-label" style={{color:'#388E3C'}}>Срочность</label>
                ...
                {urgencyOptions.map(opt => (
                  <button
                    ...
                    style={{
                      ...
                      borderColor: formData.urgency === opt.value
                        ? (opt.value === 'critical' ? '#F44336' : opt.value === 'urgent' ? '#FF9800' : '#4CAF50')
                        : '#E0E0E0',
                      background: formData.urgency === opt.value
                        ? (opt.value === 'critical' ? '#FFEBEE' : opt.value === 'urgent' ? '#FFF3E0' : '#E8F5E9')
                        : 'white',
                      borderRadius:'10px', cursor:'pointer', fontSize:'13px', fontWeight:'600',
                      color: formData.urgency === opt.value
                        ? (opt.value === 'critical' ? '#C62828' : opt.value === 'urgent' ? '#E65100' : '#2E7D32')
                        : '#666',
                      transition:'all 0.2s'
                    }}>
                    {opt.label}
