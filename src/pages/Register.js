  ...
  const handleRegister = async (e) => {
    ...
    try {
      const userData = {
        ...
        userType: isVolunteer ? 'volunteer_pending' : formData.userType,
        postsCount: 0,
        helpCount: 0,
        isVerified: false,
      };
      if (isVolunteer) {
        userData.volunteerLink = formData.volunteerLink;
      }
      if (isService) {
      ...
      <div className="auth-container">
        ...
        <div className="auth-form">
          ...
          <form onSubmit={handleRegister}>
            ...
            {/* Блок волонтёра */}
            {isVolunteer && (
              <div style={{background:'#FFF5F5', border:'2px solid #FFCDD2', borderRadius:'12px', padding:'16px', marginBottom:'16px'}}>
                <div style={{fontSize:'13px', color:'#555', lineHeight:'1.6', marginBottom:'16px', padding:'12px', background:'white', borderRadius:'8px', borderLeft:'3px solid #E8534A'}}>
                  <p style={{margin:'0 0 8px 0'}}>Уважаемые друзья, мы ни сколько не сомневаемся в Ваших благородных стремлениях помогать нашим братьям меньшим, но первоначально новые профили волонтёров будут модерироваться.</p>
                  <p style={{margin:'0 0 8px 0'}}>Поэтому важны любые подтверждающие факты Вашей волонтёрской деятельности — укажите ссылку на Ваш профиль в любой соц. сети или мессенджере.</p>
                  <p style={{margin:'0 0 8px 0'}}>Профиль волонтёра даст Вам дополнительные возможности нашего приложения.</p>
                  <p style={{margin:'0', fontWeight:'700', color:'#E8534A'}}>Огромное Вам спасибо! Проект «AWOO» ❤️</p>
                </div>
                <div className="form-group" style={{marginBottom:0}}>
                  <label>Ссылка на профиль в соцсети / мессенджере *</label>
                  <input
                    type="url"
                    name="volunteerLink"
                    value={formData.volunteerLink}
                    onChange={handleChange}
                    placeholder="https://vk.com/... или t.me/..."
                ...
                <div className="form-group">
                  ...
                  <div style={{display:'flex', flexWrap:'wrap', gap:'8px', marginTop:'6px'}}>
                    {SERVICE_CATEGORIES.map(cat => (
                      <button key={cat.value} type="button" onClick={() => toggleCategory(cat.value)} disabled={loading}
                        style={{
